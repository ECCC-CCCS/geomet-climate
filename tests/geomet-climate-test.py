...
# The following list of strings is used to avoid testing for an empty file for products containing those substrings
# We may want to not do that when we are more certain that the GetMap images we are requesting are supposed to be non empty
# e.g. by using a specific BBOX as a function of product
no_empty_check = ['_FR', '_RN', '_NT', '_PR', '_RT', '_SN', '_PEMM', '_PRMM', '_FRMM', '_PRMM', '_RNMM', '_SNMM', 'OCEAN', 'ETA_PN', 'HURRICANE']

# This is now a dict (used to be a list) so we can later choose which BBOX to check as a function of product
...
def datetime_generator(begin_dt, end_dt, time_interval):
    """Yield datetime objects between an initial and a final datetime for a specified time interval.
...
def time_interval_to_csv_timestamps(time_interval_iso_string):
    '''Takes an ISO 8601 time interval (t_begin/t_end/period) and retruns a list of comma separated timestamps.'''
    ...
    it = itertools.islice(datetime_generator(begin_dt, end_dt, int_dt), nb_intervals + 1)
...
def images_equal(im1, im2):
    '''If im1 and im2 (pngs returned by GetMap) are the same, resulting diff bbox is None so returns True
    '''
    return ImageChops.difference(im1, im2).getbbox() is None

def image_empty(im, bgcolor=None):
    '''Takes an image and an optional background color in hex format (e.g. #FF0000).
    If im has a single pixel value different from bgcolor if any supplied, it is empty.
    '''

    # If an image has uniform values, then standard deviation will be zero, in which case we check if that value != bgcolor if any
    ...
    if bgcolor:
        ...
        if sum(ImageStat.Stat(im).stddev) == 0 and image_rgb != bgcolor_rgb:
            err_str = "Constant pixel value of " + str(image_rgb) + "; Background color for NODATA = " + str(bgcolor) + " (RGB = " + str(bgcolor_rgb) + ")"
...
def check_arg(args=None):
    ...
    parser.add_argument('--output-dir', help='Specify output directory where to write results (mostly png files); default = none')
    parser.add_argument('--loglevel', help='logging level (CRITICAL, ERROR, WARNING, INFO, DEBUG)', default='ERROR', choices=uclist(['CRITICAL', 'ERROR', 'WARNING', 'INFO', 'DEBUG']))
    parser.add_argument('--mapfile', help='location of mapfile, default = none')
    parser.add_argument('--test-subset', help='Specify the test subset you want to run ', default='Default')
...
def CreateTestSubSet(TestSuite):
    ...
    #Definition of all the test running for each subset, tried to make a list but apparently  the lib is limited in terms of capabilities.
    if TestSuite == 'nightly':
        ...
        TestSubSet.addTest(GeoMetTest('test_time_enabled_yaml_getcapabilities'))
        TestSubSet.addTest(GeoMetTest('test_request_with_time'))
    elif TestSuite == 'dev':
        TestSubSet.addTest(GeoMetTest('test_layers_availability_geomet_climate'))
    elif TestSuite == 'ops':
        TestSubSet.addTest(GeoMetTest('test_layers_availability_geomet_climate'))
    elif TestSuite == 'beta':
        TestSubSet.addTest(GeoMetTest('test_layers_availability'))
...
class GeoMetTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.wms = WebMapService('http://geo.weather.gc.ca/geomet-climate', version='1.3.0', timeout=300)
        geomet_climate_yml = '/data/web/geomet-climate-nightly/latest/geomet-climate/geomet-climate.yml'
    ...
    def test_geomet_service_online(self):
        #Test Geomet Service Online : Simple Test with a generic client to make sure the service is online
        ...
        try:
            ...
            LOGGER.error('Geomet-Climate Service Online Failed : request issued : \n{}'.format(self.wms.request))

    def test_bad_layer_request(self):
        #Test Bad Layer Request : Make a bad layer request to validate an exception is thrown

        try:
            blob = self.wms.getmap(layers=['some-bad-layername'],
                                   srs=SRS,
                                   bbox=[-90, -180, 90, 180],
                                   size=(self.sizewidth, self.sizeheight),
                                   format=FORMAT,
                                   transparent=True)
            LOGGER.error('Bad Layer Request Failed : Should have return an exception')

        except Exception as e:
            error = False #Nothing to do, this is the result expected

    def test_layers_availability(self):
        #Test Layer Availability : Ensure all layers in configuration are in WMS

        #Find all the layer present in the yaml file and not in GetCapabilities
        self.wms_missing_layers = list(set(self.config['layers'].keys()) - set(self.wms.contents.keys()))

        #Log all the missing layers as errors
        if self.wms_missing_layers:
            self.layer_time_error_count += 1
            LOGGER.error('Layers Availability Failed : WMS missing layers from YAML configuration: {}'.format(
                self.wms_missing_layers))

    def test_legends_availability(self):
        #Test Legends Availability : Ensure all layer legends show up and are not empty
            ...
            if layer.parent is not None:  # leaf layer
                ...
                for style in layer.styles.values():
                    ...
                    try:
                        ...
                        LOGGER.error('Legends Availability Failed : GetLegendGraphic request failed on request : \n{}'.format(res.url))

                    #if want to skip the actual layer...
                    if key not in skip_layers:
                        img = Image.open(BytesIO(res.content))
                        is_empty = image_empty(img)
                        if ('Content-Length' in res.headers and res.headers['Content-Length'] == 0) or is_empty['is_empty']: #wut is that if ???
                            self.getlegend_empty_error_count += 1
                            LOGGER.warning('Legends Availability Warning : Empty content response or empty image with request : \n{}'.format(res.url))

    def test_styles_availability(self):
        #Test Styles Availability : Perform a test to make sure all styles are available
            ...
            for style in layer.styles.keys():
                ...
                if style not in list_layer_evaluated:
                    ...
                    try:
                        ...
                        LOGGER.error('tyle Availability Failed : Style ' + style + ' is unavailable, Style was tested with layer ' + key)
                        LOGGER.error('Failed request : {}'.format(self.wms.request))

                    #Validate if the image returned by the request is empty or not
                    img = Image.open(res)
                    is_empty = image_empty(img)

                    #if the image is empty : log a warning since some style might return blank images if no data is available (ex : FreezingRain or HURRICANE)
                    if is_empty['is_empty']:
                        LOGGER.warning('Style Availability warning : ' + style + ' is available but the returning image is empty with layer : ' + key)
                        LOGGER.warning('Failed request : {}'.format(self.wms.request))

    def test_time_enabled_yaml_getcapabilities(self):
        #Test Time Enabled Yaml GetCapabilities : Perform a test that make sure all layer in the yaml with time_enabled has a time dimemsion in the GetCapabilities
        ...
        #Get all layers and indicate if the layer has time_enabled or not (time_enabled can be : No, Yes, Future)
        for key in yml:
            try:
        ...
        #Get all layers and indicate if the layer has a time dimension or not (The tags isnt there is time_enabled is 'no')
        for key, layer in self.wms.contents.items():
        ...
        #Compare yaml and GetCapabilities, log an error on a time_enabled missmatch for a layer
        for key in yaml_layers_dict:
            if yaml_layers_dict.get(key) != getcapa_dict.get(key):
                self.gettime_enabled_error_count += 1
                LOGGER.error('Time Enabled Error : Layer ' + key + ' YAML Time enabled setup to : ' + str(yaml_layers_dict.get(key)) + ' and GetCapabilities Time Dimension setup to : ' + str(getcapa_dict.get(key)))

    def test_request_with_time(self):
        #Test Request With Time : Perform GetMap requests with a time parameter to make sure those are supported
            ...
            if group[0] not in list_group_evaluated:
                ...
                if layer.timepositions:  # set start/end times list
                    ...
                    try:
                        ...
                        blob = self.wms.getmap(layers=[key], styles=[''],
                                               srs=SRS,
                                               bbox=[-180, -90, 180, 90],
                                               size=(self.sizewidth, self.sizeheight),
                                               format=FORMAT,
                    ...
                    # If request failed, log error as time_request failed
                        ...
                        LOGGER.error('Request With Time Failed : Layer ' + key + ' failed with a time request')
                        LOGGER.error(self.wms.request)

if __name__ == '__main__':
    URL, OUTPUT_DIR, LOGLEVEL, MAPFILE, TESTSUBSET = check_arg(sys.argv[1:])
    URL = URL.rstrip('/')
    ...
    # Without next line unittest.main() will fail with AttributeError: 'module' object has no attribute 'http://...'
    sys.argv = [sys.argv[0]]
